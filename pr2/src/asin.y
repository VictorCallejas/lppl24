...
declaFunc:
      tipoSimp ID_ AP_ paramForm CP_
      {
        niv = 1;
        guardadvar = dvar;
        dvar = 0;
...
/* ---------------- SPLIT THE IF RULE INTO TWO: ifSinElse and ifConElse ---------------- */
instSelec:
        ifSinElse
      | ifConElse
      ;
...
/* ---------------- SPLIT THE FOR RULE INTO A HEADER AND A BODY ---------------- */
instIter:
      forHeader inst
      ;

/* The for header: do the checks right after reading `)`, so the line is more accurate */
forHeader:
    FOR_ AP_ expreOP PCOMA_ expre PCOMA_ expreOP CP_
    {
      if (!$3.tipo == T_VACIO) {
        checkTipoSimple($3.tipo, "La expreOp del 'for' debe ser tipo simple, arg 0");
      }
      checkLogico($5.tipo, "La expresion del 'for' debe ser logica, arg 1");
      if (!$7.tipo == T_VACIO) {
        checkTipoSimple($7.tipo, "La expreOp del 'for' debe ser tipo simple, arg 2");
      }
    }
    ;
